...
\item The probability that $z$ and $y$ are IBD can be through two ways:
  \begin{itemize}
  \scriptsize
  \item (An extreme case where we assume the common ancestor as having both alleles at a loci different; heterozygous) First way is when $z$ and $y$ are both the same copy (both pink or blue). This happens $\frac{1}{2}$ of the time since 1/4 of the time they are both blue and 1/4 both pink.
...
Let us consider a quantitative trait controlled by 5 genes in following combinations in four inbred lines of maize.

$P1 = AABBccDDee$, $P2 = AAbbCCddEE$, $P3 = aabbCCDDEE$ and $p4 = AABBccddee$

All gene contribute equally to the trait value of phenotype; i.e., $AA == BB == CC == DD == EE = 2$ and $aa == bb == cc == dd == ee = 1$. The phenotypes show complete dominance. i.e., $Aa == AA$. Now to consider crosses between various parents,
...
\begin{aligned}
&P1 & &\times &P2 \\
&AABBccDDee & &\times &AAbbCCDDEE \\
&&\downarrow&& \\
&&\text{All } AaBbCcDDEe  & \rightarrow \text{Trait value = 10} & \\
\end{aligned}
$$
...
&aabbCCddEE & &\times &AABBccddee \\
&&\downarrow&& \\
&&\text{All } AaBbCcddEe & \rightarrow \text{Trait value = 9} & \\
\end{aligned}
$$
...
&AaBbCcDDEe & & \times &AaBbCcddEe \\
&&\downarrow&& \\
&&\text{Segregating and non-segregating} & \rightarrow \text{Trait value} \leq 10 & \\
\end{aligned}
$$
