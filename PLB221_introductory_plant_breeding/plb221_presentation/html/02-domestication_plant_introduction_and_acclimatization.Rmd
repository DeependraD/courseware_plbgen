...
bibliography: ['./bibliography/02-domestication_plant_introduction_and_acclimatization.bib']
---
...
```{r weed-vs-crop, fig.cap="Perennial teosinte", fig.height=3, out.height="600px", fig.align='center'}
...
domestication_syndrome <- readxl::read_xlsx("./02-domestication_plant_introduction_and_acclimatization.xlsx", sheet = "domestication_syndrome")
DT::datatable(
...
```{r, fig.align='center', fig.cap="Teosinte maize hybrid", out.height="600px"}
...
```{r gene-pools, fig.cap="Crop gene pools; A system proposed by Harlan"}
...
```{r eval=require('DT'), tidy=FALSE}
domestication_origin <- readxl::read_xlsx("./02-domestication_plant_introduction_and_acclimatization.xlsx", sheet = "center_origin", skip=2)

DT::datatable(
  domestication_origin,
  fillContainer = FALSE, caption = "Estimated time of domestication and centre of origin of major crop species; @brown2014plant, Page 23", options = list(pageLength = 6)
) %>% 
  DT::formatStyle(columns = colnames(.), fontSize = "50%")
```
...
```{r cultivated-megacentres, fig.cap="Megacentres of cultivated plants (Zeven and Zhukovsky, 1975); @hayward2012plant, Page 37"}
# pdftools::pdf_convert("./../../references_plb/E. Sánchez-Monge (auth.), M. D. Hayward, N. O. Bosemark, I. Romagosa, M. Cerezo (eds.) - Plant Breeding_ Principles and prospects.pdf", format = "png", pages = 53, filenames = "./images/megacentres_cultivated.png", dpi = 300)
...
```{r eval=require('DT'), tidy=FALSE}
megacentres <- readxl::read_xlsx("./02-domestication_plant_introduction_and_acclimatization.xlsx", sheet = "diversity_region", skip=2)
