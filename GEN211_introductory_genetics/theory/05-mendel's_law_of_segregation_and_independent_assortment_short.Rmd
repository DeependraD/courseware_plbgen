...
- Self pollinating crop, so pollination control was not a problem.
- Easy to cross as the flowers are relatively large.
- Easy to cultivate and only took a season to raise a single generation
- Seeds are large and have good germination in general. Plants also take little space to grow.
- Varieties had many sharply defined inherited differences

# Law of segregation/law of purity of gametes

## Mendel's work

- When two different varieties were crossed for each character, mendel uniformly found that $\mathbf{F_1}$ was of only one type.
...
- For all seven characters tested, the results appeared to fit the following pattern
  1. The $\mathrm{F_1}$ derived from crosses between two different varieties showed only one of the traits and never the other.
...
```{r phenotypic-ratios, out.width=c("40%", "50%"), fig.align='center', fig.ncol = 2, fig.cap="Mendel's observations of monohybrid experiment on Pea", fig.subcap=c("Experimental outcome summary", "Observed data")}

# fig.subcap=c("Mendel obtained a 3:1 phenotypic ratio in his self-pollination of the $\\mathrm{F_1}$, on left, and a 1:1 phenotypic ratio in his cross of $\\mathrm{F_1}$ yellow with green, on right. Sample sizes are arbitrary.", "Results of All Mendel's Crosses in Which Parents Differed in One Character")
...
- A plant with a pair of alleles of different forms is called a **heterozygote**.
...
- To perform a dihybrid cross, Mendel started with two pure parental lines. One line had wrinkled, yellow seeds. 
<!-- Because Mendel had no concept of the chromosomal location of genes, we must use the dot representation to write the combined genotype initially as $r/r.Y/Y$.-->
The other line had round, green seeds, with genotype $R/R.y/y$.
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{figure}
  \begin{center}
  \includegraphics[width=0.45\linewidth]{../images/mendels_dihybrid_cross.png}
  \end{center}
...
```{r independent-assortment-prob, out.width="42%", fig.align='center'}
# pdftools::pdf_convert("../../literatures/textbooks/Introduction to Genetic Analysis/Griffiths - An Introduction to Genetic Analysis 11th Edition c2015.pdf",
#                       pages = 116,
#                       dpi = 300, format = "png",
