# Solution of numerical problems related to linkage analysis and crossing over

## Question 1

In Drosophila the allele $dp+$ determines long wings and $dp$ determines short wings. At a separate locus $e+$ determines the gray body and $e$ determines ebony body. Both loci are autosomal. Pure line of long ebony parent was crossed to pureline of short gray and produced long gray $F_1$. The $F_1$ was crossed to pure line of short ebony and the following progeny were obtained.
...
Use $\chi^2$ test to determine if these loci are linked.


## Question 2

Two different plants heterozygous for the concerned genes are self-pollinated and in $F_2$ following data was recorded:

```{r}
two_plants_f2 <- tribble(~"phenotypic_class1", ~"frequency1", ~"phenotypic_class2", ~"frequency2",
       "A_B_", 250, "C_D_", 510,
       "A_bb", 45, "C_dd", 240,
       "aaB_", 35, "ccD_", 240,
       "aabb", 70, "ccdd", 10)
two_plants_f2 %>% 
  kable(booktabs = TRUE, longtable = TRUE, col.names = rep(c("Phenotypic class", "Frequency"), 2)) %>% 
  kableExtra::add_header_above(c("Plant 1" = 2, "Plant 2" = 2)) %>% 
  kableExtra::kable_styling(font_size = 8)
```

i. Are the genes $a$ and $b$, and $c$ and $d$ linked ? If so, in which phase ?
ii. Determine the frequency of recombination between the genes a and b, and c and d.
