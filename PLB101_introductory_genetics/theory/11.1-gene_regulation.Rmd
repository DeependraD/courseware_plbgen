...
header-includes:
  - \pgfdeclareimage[height=\paperheight]{mybackground}{deep_blue_sky_parbat.jpg}
---
...
- \textbf{The \textit{lac} structural genes}\footnote[frame]{Genes that encode the primary structures of proteins required by all cells for enzymatic or structural functions are called structural genes.}
...
```{r galactosidic-bond-lactose, fig.align='center', out.width="85%"}
knitr::include_graphics("../images/beta_14_galactosidic_bond_breakage.png")
```
...
- Six potential levels for regulation of gene expression exist in eukaryotes, regulation of:
  1. activation of gene structure (DNA packaging machinery; Figure \ref{fig:dna-packaging}),
  2. transcription, 
  3. gene replication
  ...
  5. translation, and
  6. protein activity
  7. post-translational modification (PTM) (of gene via histone modification itself (epigenetic!) or other functional proteins)
- Unlike that of a polycistronic gene (lacks introns; has one transcriptional unit and one translational unit) as occurs in bacteria, eukaryotic genes are monocistronic (split genes with introns). 
...
<!-- Nucleosomal DNA is distorted into 1.65 turns of a flat, left-handed superhelix, with a radius of 42 Angstorm and a pitch of 26 A. -->

\column{0.55\textwidth}

```{r dna-packaging, out.width="50%", fig.align='center', fig.cap="The many levels of chromatin packing. Each DNA molecule has been packaged into a mitotic chomosome that is 10,000-fold shorter than its extended length. (Adapted from B. Alberts et al., Molecular Biology of the Cell, 6th ed. New York: Garland Science, 2015.)"}

knitr::include_graphics("../images/dna_packaging.png")

# In the mid-1970s several groups succeeded in isolating stable nucleoprotein particles that could be characterized by analytical ultracentrifugation. These particles were presumably the 'beads' observed in electron micrographs, which we now call nucleosomes. In 1974 a structural model based on biochemical evidence was introduced in which two copies each of histones H2A, H2B, H3, and H4 form an octameric structure. The (H3-H4)2 tetramer was proposed to coil ~200 base pairs (bp) of DNA around its periphery, while binding of the H2A-H2B dimers stabilized the octamer–DNA complexes. The DNA was no longer seen as being coated by histones but as being wrapped around an octameric histone core, forming a contiguous repetitive array separated by histone-free and nuclease-accessible 'linker' DNA.

# The four core classes of histones (H2A, H2B, H3, and H4) are highly conserved proteins. Only 2 of the 102 amino acids are different between the H4 of plants and mammals. The core histones of all eukaryotes are small (11–15 kDa), highly positively charged proteins that consist of two structurally and functionally separable domains.The N-terminal domain (NTD) or tail domain consists of ~20-35 residues. Most of the minor sequence variability occurs in the NTDs, which serve as essential determinants of chromatin condensation. The next ~80-100 residues are mostly alpha-helical and contain the 'histone fold' domains and extensions. These interact with other histones and nucleosomal DNA. In addition, histone H2A also has an unstructured C-terminal tail domain (CTD).

# Because of the instability of the histone octamer under physiological conditions, the nucleosome is not assembled by binding of an intact histone octamer to DNA. Rather, nucleosome assembly occurs via a sequential step-wise mechanism.

# For more accounts of the histone, refer to Page 50 of Molecular Biology of Assemblies and Machines on "Nucleosomes and higher order chromatin structures" section.
```
...
## Dynamic chromatin: Chromatin remodeling
...
```{r chromatin-remodeling, fig.cap="The histone octamer slides in response to chromatin-remodeling activity (such as that of the SWI–SNF complex), in this case exposing the DNA marked in red.", out.width="60%"}
# pdftools::pdf_convert("../../literatures/textbooks/Introduction to Genetic Analysis/Griffiths - An Introduction to Genetic Analysis 11th Edition c2015.pdf",
#                       pages = 470,
#                       dpi = 300, format = "png",
...
- A higher density of linker histones is normally associated with the electron-dense heterochromatin, which is associated with transcriptional repression, whereas the less electron-dense and transcriptionally active euchromatin has a lower concentration of linker histones.
- Chromatin remodeling complexes move, reconfigure, or eject nucleosomes to provide the chromatin fluidity needed for chromosomal processes such as gene regulation, DNA repair, replication, and recombination.
- **Epigenetic** changes include variant histones, histone PTMS, DNA methylation, and the action of noncoding RNAs. Epigenetic mechanisms often involve both 'writers' and 'readers' of epigenetic marks.

<!-- DNA methylation particularly of CpG islands (regions rich in CpG dinucleotides) is another epigenetic marker that results in widespread gene silencing. -->

<!-- Specific histone PTMs are recognized by domains such as the bromodomains interacting with acetylated histones or the chromodomains with methylated histones. -->

## Transcriptional regulation of gene expression

```{r eukaryotic-transcription, fig.cap="Formation of the preinitiation complex usually begins with the binding of the TATA-binding protein (TBP), which then recruits the other general transcription factors (GTFs) and RNA polymerase II to the transcription start site. Transcription begins after the phosphorylation of carboxyl terminal domain (CTD) of RNA polymerase II.", out.width="30%"}
# pdftools::pdf_convert("../../literatures/textbooks/Introduction to Genetic Analysis/Griffiths - An Introduction to Genetic Analysis 11th Edition c2015.pdf",
#                       pages = 330,
#                       dpi = 300, format = "png",
