...
## Holiday-Junction (H-J) approach in Recombination

- Refer to Youtube videos (available in "media" directory) on
  - Holiday junction resolution by Oxford University Press
  - Homologous recombination_holliday junction resolution
...
## Because conformation matter!

```{r conformation-matter, fig.align='center', out.width="60%"}
knitr::include_graphics("../images/samuel_l_d_jackson.jpg")
```
...
$$
\begin{aligned}
&\textrm{P} & v^+/v^+.cv/cv.ct/ct\hspace{1cm}&\times\hspace{1cm} v/v.cv^+/cv^+.ct^+/ct^+ \\
& &&\downarrow \\
& \textrm{Gametes} & v^+.cv.t\hspace{1cm}&\phantom{\times}\hspace{1cm}v.cv^+.ct^+ \\[1cm]
...
\small
\begin{itemize}
\item Trihybrid females are testcrossed with triple recessive males:
\end{itemize}

$$
\small
\begin{aligned}
& v^+/v.cv^+/cv.ct^+/ct &\times & v/v.cv/cv.ct/ct \\
& \mathrm{F_1~trihybrid~} {\large \female} && \mathrm{Tester~} {\large \male}
\end{aligned}
$$

\begin{itemize}
\footnotesize
\item From any trihybrid, only 2 x 2 x 2 = 8 gamete genotypes are possible. 
\item They are the genotypes seen in the testcross progeny.
\item Based on observation of progeny, following genotypes/phenotypes/gamete-types have been noted (Table \ref{tab:progeny-trihybrid}) at given frequencies.
\end{itemize}

\column{0.5\textwidth}

\textbf{Gamete types (equal segregation and independent assortment)}
...
sloped,
root/.style = {font=\small, thick, shape=circle,
    draw, align=center, drop shadow,
    top color=white, bottom color=blue!20},
next/.style = {font=\small, text width=4mm, inner sep=1pt, thick, shape=circle,
    draw, align=center, drop shadow,
    top color=white, bottom color=blue!20},
...
tribble(~"Gametes", ~"Progenies",
        ...
        "$v.cv^+.ct$", 3,
        "$v^+.cv.ct^+$", 5) %>% 
  knitr::kable(booktabs = TRUE, escape = FALSE, caption = "Count of progeny phenotypes from the testcross between F1 and recessive tester male.") %>% 
  kableExtra::kable_styling(position = "center")
```

##

```{r rehydrate-for-cry, fig.align='center'}
# fortunately, crying part should be over by now
# most of the job is done
# but save some water because we have yet to describe
# some anomalies in calculating the distance
knitr::include_graphics("../images/rehydrate.jpg")
```
...
```{r double-recombinants, out.width="70%", fig.align='center'}
# pdftools::pdf_convert("~/Desktop/BSc_Ag_lectures/literatures/textbooks/Introduction to Genetic Analysis/Griffiths - An Introduction to Genetic Analysis 11th Edition c2015.pdf", pages = 166, dpi = 300,
...
```{r}
# obtaining the larger map distance without accounting for crossovers is like
# getting the shower when you want to wash your hands in the tap
# you get water, but not the correct way you want it
knitr::include_graphics("../images/shower_instead_of_tap.jpg")
```
