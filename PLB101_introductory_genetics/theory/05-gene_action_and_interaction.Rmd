...
- Some phenotype develop through gene interaction between two or more genes. i.e. the phenotypic ratio deviates from the expected Mendelian phenotypic ratio. 
- There are a variety of phenotypic ratios which are the result of a variety of gene interaction. Steps in inferring gene interaction :
  ...
  3. Combine the mutants in pairs (the process is detailed in a Complementation test\footnote[frame]{Refer to lecture notes on 'Nature of gene', Course: Genetics of population, $5^{th}$ semester, BScAg]) to form double mutants to see if the genes interact.}
...
<!-- (Refer to Chapter 12 on 'Gene interaction and selection', Plant Breeding Reviews, Volume 24, Part 1.) -->
...
combination_genotypes <- crossing(gamete_types, gamete_types, .name_repair = "unique") %>% 
  # group_by(gamete_types)
  rename(gamete_types = gamete_types...2) %>% 
  unite(col = "genotype", remove = FALSE, sep = "") %>% 
  spread(key = gamete_types...1, value = genotype)
...
```{r comb-types-poultry, fig.cap="Comb types in poultry; (A) single, (B) Rose (C) Pea (D) Walnut.", out.width="80%"}
knitr::include_graphics("../images/comb-types-poultry.png")
```
...
map2_dfc(.x = (combination_genotypes %>%
         ...
         .f = ~kableExtra::cell_spec(.x, background = .y, format = "latex")) %>%
  add_column(.before = 1, "Gamete" = colnames(.)) %>% 
  kable(booktabs = TRUE, format = "latex", escape = FALSE, linesep = "", caption = "This phenomena is also known as recessive epistasis, in that one of the two genes shows epistatic action on other when present in recessive state. Like complementary gene action, epistatic effect is observed also due to the genes occuring in the same biochemical pathway; only difference being that intermediate biochemical product of the pathway produce a distinct phenotype so long as the epistatic gene is dominant. Once the epistatic gene (located further upstream in the pathway that leads to intermediate product) occurs in recessive form, it disrupts intermediate product formation. (Phenomena is well illustrated \\textit{Collinsia} flower; Refer to lecture slide 'Nature of Gene' of 5th semester.) ") %>%
  kableExtra::kable_styling(font_size = 6) %>% 
  kableExtra::column_spec(1:5, width = c("2em", "1.8em", "1.8em", "1.8em", "1.8em"))
...
(Refer to Chapter 2 on "Epistasis and plant breeding", Plant Breeding Reviews, Volume 21.)
...
## Complementary genes: genes acting on same pathway
...
```{r gene-in-same-pathway, out.width="85%", fig.align='center'}
...
map2_dfc(.x = (combination_genotypes %>%
         ...
         .f = ~kableExtra::cell_spec(.x, background = .y, format = "latex")) %>%
  add_column(.before = 1, "Gamete" = colnames(.)) %>% 
  kable(booktabs = TRUE, format = "latex", escape = FALSE, linesep = "", caption = "This phenomena is also known as dominant epistasis, in that one of the two genes shows epistatic action on other when present in dominant state. Gene 'A' is epistatic to gene 'B'. 'A' masks the effect of 'B' with its own phenotype when they both occur together, while B, on its own, shows distinct phenotype.") %>%
  kableExtra::kable_styling(font_size = 6) %>% 
  kableExtra::column_spec(1:5, width = c("2em", "1.8em", "1.8em", "1.8em", "1.8em"))
...
map2_dfc(.x = (combination_genotypes %>%
         ...
         .f = ~kableExtra::cell_spec(.x, background = .y, format = "latex")) %>%
  add_column(.before = 1, "Gamete" = colnames(.)) %>% 
  kable(booktabs = TRUE, format = "latex", escape = FALSE, linesep = "", caption = "Genes show doubly epistatic effects. Gene 'A' when present in dominant state inhibits the phenotypic expression of 'B' and Gene 'B' when present in recessive state inhibits the phenotypic expression of 'A'.") %>%
  kableExtra::kable_styling(font_size = 6) %>% 
  kableExtra::column_spec(1:5, width = c("2em", "1.8em", "1.8em", "1.8em", "1.8em"))
```
