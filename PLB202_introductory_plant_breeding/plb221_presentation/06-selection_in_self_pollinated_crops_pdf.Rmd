...
## Refer to following Chapters and references for ideas on selection

# - Chapter 6 - Selection Theory, Quantitative Genetics in Maize Breeding (2010)
# - Chapter 11 - SELECTION: I. THE RESPONSE AND ITS PREDICTION, Introduction to Quantitative Genetics (1996)
...
- A group of genetically similar plants which may be identified and differentiated (by some means) from other group of genetically similar plants.
- Essential characteristics (internalized also by the International Union for the Protection of New Varieties of Plants (UPOV)):
...
- Factors affecting types of cultivars/varieties developed
  ...
  - Cost of the product used for commercial planting relative to it's performance and value (in vegetables were cost per unit seed is high and the returns per unit seed can easily exceed that provided by conventionally breed seeds, hybrids are preferred )
  - Economic value of the crop
  - Distribution system
  - Environment where crop is produced

<!-- (Source: Lecture 07 - Breeding self pollinated crops; Vernon Gracen, Professor Emeritus, Department of Plant Breeding and Genetics, CALS, Cornell University) -->

<!-- Cultivar is a preferred term for all types of distinguishably uniform genotypes but variety is defined in numerous ways (notified under national system for distribution and use). -->

## Types of cultivars/varieties and determinants of breeding procedure

(For extended discussion with examples, refer to the section "Types of cultivars/varieties",  Lecture 3: Modes of reproduction and pollination control)

1. Self-pollinated crops are usually pure-line cultivars/varieties but can be hybrids
2. Cross-pollinated crops are usually open pollinated varieties, synthetic varieties or hybrids
3. Clonal cultivars are used most often with species that have high value, are highly heterozygous and/or easily asexually propagated.
...
# Cultivar/variety breeding outline
...
- Pure-line varieties of self pollinated crops
  - select or develop (or not necessary in mass selection procedure) genetically diverse segregating populations
...
- Synthetic varieties and varietal crosses of cross pollinated crops
  - select and cross a number of superior genotypes in all combinations to develop a synthetic variety or select and cross open-pollinated varieties that display good combining abilities to develop varietal hybrids
  - usually involves some type of recurrent selection

\column{0.3\textwidth}

```{r rice-plant, fig.align='center', out.width="35%"}
knitr::include_graphics("./images/rice_plant.png")
```

\ecolumns

<!-- In recurrent selection, the idea is to allow recombination of genotypes (due to intermating) followed by selection. This being repeated a number of times. -->

##

- F1 hybrids of both cross and self-pollinated crops
  - develop inbred lines from segregating populations by selfing or inbreeding to homozygosity
...
1. A mixed population of self-pollinated plants is composed of plants with different homozygous genotypes
...
2. Exploit the homozygous nature of individual plants
3. Focus on plants **within** populations
4. Qualitatively inherited characters receive most attention

## Source population for improving self pollinated crops

- Originally landraces or varieties found in isolated echological niches were used successfully to develop improved varieties
- These land races contained a mixture of several types of lines arising as a result of spontaneous mutations and natural out-crossing
- They contained some desirable but often many undesirable alleles
- Extensive selection from landraces reduced the diversity and mined out the desirable alleles
- New, useful genetic variation was created via hybridization

## Creation and loss of genetic diversity

```{r creation-loss-diversity}
list("Variety A x Variety B" = paste0("F", 1:8)) %>% 
  as_tibble() %>% 
  mutate("Percentage heterozygosity" = (1/2)^(row_number()-1)) %>% 
  mutate("Percentage homozygosity" = 1 - `Percentage heterozygosity`) %>% 
  mutate(across(.cols = contains("percentage"), scales::label_percent())) %>% 
  knitr::kable(booktabs = TRUE, caption = "Creation of variation due to breeding (non-identical) individuals and consequences of inbreeding by selfing.") %>% 
  kableExtra::kable_styling(font_size = 8) %>% 
  kableExtra::column_spec(column = 1:3, width = c("10em", "12em", "12em"))
```
...
$$
  ...
  - Where,
    - $m$: Number of generations past since F1 generation; i.e $m = 0$ at F1.


##

```{r}
readxl::read_xlsx("./data/increase_in_homozygosity.xlsx", range = "E2:J14") %>% 
  knitr::kable(col.names = c(" ", "AA", " ", "aa", "Heterozygosity", "Homozygosity")) %>% 
  kableExtra::kable_styling()
```


<!-- Why do we multiply proportion of each homozygote in previous genotype by 4 to obtain the present generation homozygote-only selfed progeny number? -->
<!-- This is because, in a fair world, if a heterozygote can multiply to give rise to 4 individuals while we are protecting all of its progenies (perfect population size: 4), we also need to provide homozygote the equal chance to procreate and reproduce. At least nature does so. -->


## Mass selection

- Mass selection (originally described by Danish biologist W. Johansen, 1903 working on beans) is an example of selection from a biologically variable population in which differences are genetic in origin. 
...
- Where applicable, single pods from each plant may be picked and bulked for planting. For cereal species, the heads may be picked and bulked.

##

```{r mass-selection, fig.cap="Mass selection in self pollinated crops", fig.align="center", out.width="48%"}
...
- A pure line suggests that a cultivar has \alert{identical alleles at all loci}.
- The theory was developed in 1903 by the Danish botanist Johannsen.

\column{0.65\textwidth}
```{r johannsen-bean, fig.cap="The development of the pure line theory by Johannsen", fig.align="center", out.width="75%"}
...
```{r pureline-selection, fig.cap="Generalized steps in breeding by pure-line selection", fig.align="center", out.width="56%"}
...
## Genetic issues of pureline selection
...
```{r pedigree-selection, fig.cap="Generalized steps in breeding by pedigree selection", fig.align="center", out.width="64%"}
...
Refer to Professor Friedrich Longin's video on Pedigree selection: Example of spelt wheat.

# Bulk selection

##

```{r bulk-selection, fig.cap="Generalized steps in breeding by bulk selection", fig.align="center", out.width="56%"}
...
- Can be carried on for many years with little effort by the breeder\footnote[frame]{\scriptsize The term "bulk selection" is often times used to refer to a improved form of "mass selection" wherein, we carry out specific crosses to compose the source population.}

<!-- Afterall, like cycles of simplified mass selection procedure, bulk selection method is based upon natural selection (but may also be assisted by low intensity negative selection pressure) in early segregating generations. -->

## Disadvantages of bulk selection
...
- Final genotypes may be able to withstand environmental stress, but may not be highest yielding
...
```{r dominant-backcross-transfer, fig.cap="Generalized steps in breeding a dominant trait by the backcross method.", fig.align="center", out.width="85%"}
...
```{r recessive-backcross-transfer, fig.cap="Generalized steps in breeding a recessive trait by the backcross method.", fig.align="center", out.width="85%"}
...
- Where,
  ...
  \end{itemize}
- Same relation also gives the proportion of homozygous individuals in any generations of recurrent selfing. For example, for $3^{rd}$ BC generation,
...
In another way, the proportion of the donor genes is reduced by 50% following each generation of backcrossing. This is obtained by the relationship $\left(\frac{1}{2}\right)^{m+1}$, where m is the number of crosses and backcross to the parent. For example, in the BC4, the value is $\left(\frac{1}{2}\right)^{5} = 3.125\%$. To obtain the percentage of homozygotes for alleles of recurrent parents in any generation, the mathematical relationship is:
...
## Genetic advance in backcross breeding
...
4. Why not backcross in $5^{th}$ year (again) (in dominant gene transfer) ?
- Because progeny testing should be done here to enable selection as early as possible. This is because if you progeny test too late while continuing backcrossing, it could be long before you realize that lines (heterozygous or homozygous dominant) that have been developed might not contain suitable combination of genes for quantitative characters such as yield.
