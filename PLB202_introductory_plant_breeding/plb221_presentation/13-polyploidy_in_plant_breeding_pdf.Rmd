...
<!-- (Refer to Chapter 4 on 'Terminology for polyploids based on cytogenetic behavior: consequences in genetics and breeding', Plant Breeding Reviews, Volume 26.) -->
...
```{r ploidy-grape, fig.cap="Polyploids in grape", out.width="80%"}
...
```{r wheat-ploidy, fig.cap="Polyploidization in common wheat", out.width="88%"}
...
- It should be pointed out that autoploidy and alloploidy are extreme forms of polyploidy. Intermediates occur between them on a continuum of genomic relationships. C.L. Stebbins called the intermediates segmental alloploids.
- Polyploids are named such that the prefix to the standard suffix (ploid) refers to the basic chromosome set. 
- For example "triploid" refers to a cell with three genomes (3x) while "hexaploid" refers to a cell with six genomes (6x).

```{r ploidy-nomenclature, fig.cap="Ploidy nomenclature", out.width="80%"}
...
```{r polyploidy-advantages,fig.align='center', out.width="55%"}
knitr::include_graphics("./images/polyploidy_advantages.jpg")
```
...
```{r ploidy-genetics, fig.cap="Genetics of polyploidy", out.width="98%"}
...
- Number of phenotypes observed depends on dominance relationship of A and a. If allele A is completely dominant to allele a, there would be only two phenotypes. If dominance is incomplete or the effect of allele A is cumulative, there could be up to five phenotypes.
...
```{r gamete-frequency-ploids, fig.cap="Autotetraploid assortment of gamete", out.width="42%"}
...
\bcolumns
\column{0.5\textwidth}
```{r dominance-phenotypes, fig.cap="Dominant and recessive phenotypes of an autotetraploid cross", out.width="88%"}
knitr::include_graphics("./images/dominance_and_phenotype.png")
```

\column{0.5\textwidth}

```{r inbreeding-effects, fig.cap = "Effects of inbreeding in autoploids", out.width="88%"}
...
```{r natural-alloploids, fig.cap="Natural alloploidization of Brassica", out.width="99%"}
...
```{r production-allopolyploids, fig.cap = "Production of allopolyploids", out.width="38%"}
knitr::include_graphics("./images/production_of_alloploids.png")
```

## Aneuploidy

```{r aneuploids-nomenclature, fig.cap="Nomenclature of aneuploids", out.width="74%"}
...
## Use of haploid inducer lines in maize

```{r doubled-haploids-maize-production, fig.cap="Maize doubled haploid system is an example of huge success in production of homozygous lines in significanty short period of time. Although DH technology is used with success in maize and other crop species, there are limitations.", out.width="80%"}
knitr::include_graphics("./images/dh_maize_scheme.png")
```

## Anther culture

<!-- ```{r} -->
<!-- # https://dhmaize.cimmyt.org/ -->
<!-- knitr::include_graphics("./images/doubled_haploid_production_maize.png") -->
<!-- ``` -->
...
```{r advantages-dh, out.width="70%", fig.align='center', out.width="92%"}
knitr::include_graphics("./images/advantages_dh.jpg")
```
...
<!-- (Refer to Chapter 3 on "Doubled haploids in genetics and plant breeding", Plant Breeding Reviews, Volume 25.) -->
