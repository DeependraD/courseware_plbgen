...
```{r gen-phen-diversity}
tribble(
  ~"genotype_male", ~"genotype_female", ~"phenotypic_values_y", ~"phenotypic_values_p1", ~"phenotypic_values_p2", ~"phenotypic_values_f1", ~"phenotypic_diversity", ~"heterosis_mp", ~"heterosis_bp", ~"heterosis_heterobeltiosis", 
  "AABBCCDD", "aabbccdd", 4, 8, 0, 8, 8, 4, 0, 2, 
  "AABBccdd", "abbccDD", 4, 6, 2, 8, 4, 4, 2, 2, 
  "AABBccdd", "aabbCCDD", 4, 4, 4, 8, 0, 4, 4, 2, 
  "AABBCCDD", "aabbccDD", 3, 8, 2, 8, 6, 3, 0, 2, 
  "AABBCCDD", "aabbCCDD", 2, 8, 4, 8, 4, 2, 0, 2, 
  "AABBCCDD", "aaBBCCDD", 1, 8, 6, 8, 2, 1, 0, 2
) %>%
  knitr::kable(booktabs = TRUE, 
               col.names = str_remove_all(str_extract(colnames(.), 
                                                      "(?=_[:alpha:]*)_([[:alpha:]\\d])*$"), "_")) %>%
  kableExtra::add_header_above(c("Genotype" = 2, "Phenotypic values" = 4, "Genotypic" = 1, "Heterosis" = 3)) %>% 
  kableExtra::kable_styling(latex_options = "striped", position = "center", font_size = 8)
```
